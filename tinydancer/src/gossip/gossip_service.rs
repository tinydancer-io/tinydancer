...
impl ClientService<GossipConfig> for GossipService {
    ...
    fn new(config: GossipConfig) -> Self {
        let node = Node::new(config.gossip_nodes);
        ...
        let gossip_handle = tokio::spawn(async move {
            ...
            let gossip_thread = tokio::spawn(async {});
            ...
            let thread_handles = vec![receiver, listener, gossip_thread, sender];
...
mod tests {
    ...
    async fn send_ping() {
        ...
        let packet = packet::Packet::from_data(
            Some(&SocketAddr::new(
